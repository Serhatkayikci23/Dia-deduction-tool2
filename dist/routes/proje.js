...
export async function setupProje(data) {
  ...
  `);
    await db.query(`ALTER TABLE projeler ADD COLUMN IF NOT EXISTS proje_suresi NUMERIC DEFAULT 0`);
    await db.query(`
  ...
  `);
    ...
    for (const p of limitliData) {
        await db.query(`INSERT INTO proje_personel (tckimlikno, personeladisoyadi, persdepartmanaciklama, gorevi)
...
projeRouter.get("/personel", async (req, res) => {
    const result = await db.query(`SELECT * FROM proje_personel ORDER BY personeladisoyadi`);
...
projeRouter.post("/projeler", async (req, res) => {
    const { proje_adi, proje_kodu, baslangic_tarihi, bitis_tarihi, renk, personel_gideri_carpani, proje_suresi, } = req.body;
    const result = await db.query(`INSERT INTO projeler (proje_adi, proje_kodu, baslangic_tarihi, bitis_tarihi, renk, personel_gideri_carpani, proje_suresi)
...
projeRouter.put("/projeler/:id", async (req, res) => {
    const { proje_adi, proje_kodu, baslangic_tarihi, bitis_tarihi, renk, personel_gideri_carpani, proje_suresi, } = req.body;
    const result = await db.query(`UPDATE projeler SET proje_adi=$1, proje_kodu=$2, baslangic_tarihi=$3,
...
projeRouter.post("/oranlar", async (req, res) => {
    ...
    const result = await db.query(`INSERT INTO personel_proje_oran (tckimlikno, proje_kodu, oran)
     VALUES ($1,$2,$3) ON CONFLICT (tckimlikno, proje_kodu) DO UPDATE SET oran=EXCLUDED.oran
     RETURNING *`, [tckimlikno, proje_kodu, oran]);
    res.json(result.rows[0]);
});
projeRouter.get("/excel", async (req, res) => {
    const personelRes = await db.query(`SELECT * FROM proje_personel ORDER BY personeladisoyadi`);
    ...
    const getColor = (renk) => COLORS[renk?.toUpperCase()] ?? "FFD9D9D9";
    const thin = { style: "thin" };
    function styleCell(cell, opts = {}) {
        const { bold = false, fontSize = 9, bgColor, alignH = "center", fontColor = "FF000000", border = true, } = opts;
        cell.font = {
            bold,
            size: fontSize,
            name: "Arial",
    ...
    ["TC KİMLİK NO", "PERSONEL ADI SOYADI", "DEPARTMAN", "GÖREVİ"].forEach((h, i) => {
    ...
    res.setHeader("Content-Disposition", "attachment; filename=proje_personel.xlsx");
