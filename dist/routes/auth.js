...
authRouter.post("/login", async (req, res) => {
    ...
    try {
        const result = await db.query("SELECT * FROM kullanicilar WHERE email = $1", [email.toLowerCase().trim()]);
...
authRouter.get("/kullanicilar", async (req, res) => {
    const result = await db.query("SELECT id, ad, email, olusturma FROM kullanicilar ORDER BY id");
    res.json(result.rows);
});
authRouter.get("/kullanicilar/:id", async (req, res) => {
    const { id } = req.params;
    try {
        const result = await db.query("select * from kullanicilar where id = $1", [id]);
...
authRouter.post("/kullanicilar/ekle", async (req, res) => {
    ...
    await db.query("INSERT INTO kullanicilar (ad, email, sifre_hash) VALUES ($1, $2, $3)", [ad, email.toLowerCase().trim(), hash]);
