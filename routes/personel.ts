...
export async function setupPersonel(data: any[]) {
...
personelRouter.get("/detay/:proje_kodu", async (req, res) => {
  const result = await db.query(
    `SELECT * FROM personel_proje_detay WHERE proje_kodu=$1`,
    [req.params.proje_kodu],
  );
  res.json(result.rows);
});
  ...
  await db.query(
    `INSERT INTO personel_proje_detay
     ...
     ON CONFLICT (tckimlikno, proje_kodu) DO UPDATE SET
       aylikust=$3, argeorani=$4, agi=$5, gvorani=$6`,
    [tckimlikno, proje_kodu, aylikust, argeorani, agi, gvorani],
  );
  res.json({ ok: true });
});
...
personelRouter.get("/excel", async (req, res) => {
  const personelRes = await db.query(
    `SELECT * FROM personel ORDER BY personeladisoyadi`,
  );
  const projelerRes = await db.query(`SELECT * FROM projeler ORDER BY id`);

  const bordroRes = await db.query(`SELECT * FROM bordro_detay`);
  const bordroMap: Record<string, any> = {};

  for (const b of bordroRes.rows) bordroMap[b.tckimlikno] = b;
  ...
  for (const proje of projelerRes.rows) {
    ...
    const detayRes = await db.query(
      `SELECT * FROM personel_proje_detay WHERE proje_kodu=$1`,
      [proje.proje_kodu],
    );
    const detayMap: Record<string, any> = {};
    for (const d of detayRes.rows) detayMap[d.tckimlikno] = d;
