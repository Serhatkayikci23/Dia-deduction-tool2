...
export async function setupBordro(data) {
...
`);
    for (const p of data) {
        await db.query(`INSERT INTO bordro_personel (tckimlikno, personeladisoyadi, persdepartmanaciklama, gorevi)
...
bordroRouter.put("/personel/:tckimlikno", async (req, res) => {
    ...
    const { argegun, digergun, toplamgun, bruttemel, fazlamesai, aylikust, argeorani, agi, gvorani, damgaterkin, } = req.body;
    await db.query(`INSERT INTO bordro_detay (tckimlikno, argegun, digergun, toplamgun, bruttemel, fazlamesai, aylikust, argeorani, agi, gvorani, damgaterkin)
...
bordroRouter.get("/excel", async (req, res) => {
  ...
  `);
    ...
    const r_isv_arge = parseFloat(req.query.sgk_isv_arge) || 0.2175;
    const r_isvisz_arge = parseFloat(req.query.sgk_isvisz_arge) || 0.02;
    const r_argematrah = parseFloat(req.query.sgk_argematrah) || 0.05;
    const r_sgk5746 = parseFloat(req.query.sgk_5746) || 0.08375;
    const r_isci2 = parseFloat(req.query.sgk_isci2) || 0.14;
    const r_issizlik2 = parseFloat(req.query.sgk_issizlik2) || 0.01;
    ...
    const headerLabels = {
        ...
        damgaterkin: "5746 SAYILI KANUN KAPSAMINDA DAMGA TERKİN EDİLECEK DAMGA VERGİSİ",
    ...
    res.setHeader("Content-Disposition", "attachment; filename=aylik_bordro.xlsx");
